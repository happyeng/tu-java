...
public class TopoRunner extends Runner {
    ...
    public void build(){
       // IPV6 或 IPV4
        ...
        // 2. 读取空间文件（仅用于空间解析的最小BDD使用）
        System.out.println("读取空间文件...");

        String spaceFile = null;
        try {
            spaceFile = Configuration.getConfiguration().getSpaceFile();
        } catch (Exception e) {
            System.err.println("无法获取空间文件配置: " + e.getMessage());
            spaceFile = "space.txt";
        }

        if (spaceFile != null) {
            if(isIpv6 || isIpv4withS) {
                Device.readOnlySpaceFileIPV6(spaceFile);
            } else {
                Device.readOnlySpaceFile(spaceFile);
            }
        } else {
            System.err.println("空间文件路径为null，跳过空间文件读取");
        }

        // 3. 解析空间数据到BDD（最小使用）
        System.out.println("解析空间数据...");
        if(!(isIpv6|| isIpv4withS)) {
            srcNet.srcDvNetParseAllSpace(Device.spaces);
        } else {
            srcNet.srcDvNetParseAllSpaceIPV6(Device.spacesIPV6);
        }
        srcNet.setSpaceParsed(true);

        // 4. 构建TopoNet结构
        genTopoNet();

        System.out.println("总节点数: " + devices.size());
        System.out.println("S0节点数: " + network.edgeDevices.size());
        System.out.println("TopoNet总数: " + topoNetMap.size());
        System.out.println("每源节点最大路径数: " + maxPathsPerSrcNode + " 条");
        System.out.println("最大路径跳数: 10 跳");
        System.out.println("验证超时时间: " + (verificationTimeoutMillis/1000/60) + " 分钟");
        System.out.println("使用直接转发表查找（无BDD编码）");
        System.out.println("结果格式: <源设备> <目标设备> <true/false>");
        System.out.println("只输出每对设备的可达性结果");

        System.out.println("运行器构建结束!!");
    }

    public void start() {
        System.out.println("开始使用交集方法进行基于路径的验证");
        System.out.println("验证策略: 逐跳查找可达网段,求交集确定真正可达的网段");
        System.out.println("结果格式: <源设备> <目标设备> <true/false>");
        System.out.println("只输出每对设备的可达性结果");
        ...
        if (enableNPBDD) {
            System.out.println("\n[NP-BDD] 重置统计信息...");
            BDDPredicateRegistry.getInstance().resetStats();
            BDDPredicateCache.getInstance().resetStats();
        ...
        try {
            ...
            for (TopoNet topoNet : topoNetMap.values()) {
                topoNetCount++;
                System.out.println("处理TopoNet " + topoNetCount + "/" + topoNetMap.size() +
                                 ": " + topoNet.dstDevice.name);

                // 串行处理每个TopoNet
                processTopoNet(topoNet, sharedQueueBDD);

                // 刷新结果文件
                flushResults();

                System.out.println("TopoNet " + topoNet.dstDevice.name + " 处理完成");
            }

            System.out.println("所有TopoNet处理完成");
            ...
            try {
                ...
                if (resultWriter != null) {
                    resultWriter.close();
                    System.out.println("验证结果已写入文件: verification_results.txt");
                }
            } catch (IOException e) {
                System.err.println("关闭结果文件失败: " + e.getMessage());
            }
            ...
            // 打印NP-BDD详细统计
            if (enableNPBDD && printDetailedStats) {
                printSimpleNPBDDStats();
            } else if (enableNPBDD) {
                // 只打印简要统计
                printSimpleNPBDDStats();
            }
    ...
    private void topoGenNode(TopoNet topoNet){
        for(Device device : devices.values()){
            ...
            if(TopoNet.edgeDevices.contains(device)){
                if(device == topoNet.dstDevice) { // 终端节点
