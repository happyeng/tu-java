...
public class Device {
    ...
    public void encodeRuleToLecFromScratchToFinish(DVNet dvNet) {
        ...
        BDDPredicateCache cache = useCache ? BDDPredicateCache.getInstance() : null;
        ...
        for (Rule rule : rules) {
            ...
            if (useCache) {
                ...
                Integer cachedMatch = cache.getL2Encode(l2Key);
                if (cachedMatch != null) {
            ...
            if (useCache) {
                if (portPredicate.containsKey(action)) {
                    ...
                    portPredicate.put(action, tsbdd.ref(tmpHit));
                }
            } else {
                if (portPredicate.containsKey(action)) {
                    int newPredicate = tsbdd.orTo(portPredicate.get(action), tmpHit);
                    portPredicate.put(action, newPredicate);
                } else {
                    portPredicate.put(action, tsbdd.ref(tmpHit));
                }
            }
        ...
        // ========== 生成LEC集合 ==========
        // 【关键修复】Lec构造函数: Lec(ForwardAction, int) — 第一参数是ForwardAction, 第二参数是int
        HashSet<Lec> tmpLecs = new HashSet<>();
        for (Map.Entry<ForwardAction, Integer> kv : portPredicate.entrySet()) {
            if (kv.getValue() != 0) {
                // 正确: Lec(ForwardAction forwardAction, int predicate)
                tmpLecs.add(new Lec(kv.getKey(), kv.getValue()));
            }
        }
        Device.globalLecs.put(name, tmpLecs);
