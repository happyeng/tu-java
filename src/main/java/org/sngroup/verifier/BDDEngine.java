...
//        }
    ...
    /**
     * 从另一个BDDEngine复制配置和必要数据
     * 用于在并行验证中初始化新的BDD引擎实例
     *
     * @param source 源BDD引擎
     */
    public void copyFrom(BDDEngine source) {
        if (source == null) {
            System.err.println("[BDDEngine] copyFrom: source为null");
            return;
        }
        ...
        try {
            ...
            if (this.bdd == null) {
                // TSBDD需要一个BDD对象作为参数
                BDD newBDD = new BDD(500000, 1000000);
                this.bdd = new TSBDD(newBDD);
            }
            // ===== 修复8结束 =====
            ...
            System.out.println("[BDDEngine] copyFrom 完成，BDD实例已初始化");

        } catch (Exception e) {
            System.err.println("[BDDEngine] copyFrom 失败: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private void initializeVariables() {
        ...
        // 初始化变量数组（这些通常是静态的，但为了安全起见）
        if (protocol == null) {
            protocol = new int[protocolBits];
            for (int i = 0; i < protocolBits; i++) {
                protocol[i] = protocolStartIndex + protocolBits - 1 - i;
            }
        }
        ...
        if (srcPort == null) {
            ...
            for (int i = 0; i < portBits; i++) {
                srcPort[i] = srcPortStartIndex + portBits - 1 - i;
            }
        }
        ...
        if (dstPort == null) {
            dstPort = new int[portBits];
            for (int i = 0; i < portBits; i++) {
                dstPort[i] = dstPortStartIndex + portBits - 1 - i;
            }
        }
        ...
        if (srcIP == null) {
            ...
            for (int i = 0; i < ipBits; i++) {
                srcIP[i] = srcIPStartIndex + ipBits - 1 - i;
            }
        }
        ...
        if (dstIP == null) {
            dstIP = new int[ipBits];
            for (int i = 0; i < ipBits; i++) {
                dstIP[i] = dstIPStartIndex + ipBits - 1 - i;
            }
        }
